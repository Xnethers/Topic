...
{
    if (islock && other.gameObject.tag == "Player")
    {
        if (_bag.use_key(key))
        { islock = false; };
    }
    ...
    { _animator.SetBool("open", true); _audioSource.PlayOneShot(open_sound); }
}

public void OnTriggerExit(Collider other)
{
    if (other.gameObject.tag == "Player")
    { _animator.SetBool("open", false); _audioSource.PlayOneShot(closs_sound); }
